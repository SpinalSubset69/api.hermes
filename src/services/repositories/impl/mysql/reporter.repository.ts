...
export class ReporterMySqlRepository implements IReporterRepository {
  ...
  public async getCountArticlesByReporteridArticles(reporter_id: number): Promise<number> {
  ...
  public async store(entry: Reporter): Promise<void> {
    //hash password
    const hashedPassword = SHA('SHA256').update(entry.password).digest('base64');
  ...
  public async findReporterArticlesBasedOnId(reporter_id: number, pageSize:number = 5, page:number = 1): Promise<Article[] | null> {
